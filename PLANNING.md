...
- ✅ **개인정보처리방침** — `/privacy.html`, 앱 우하단 링크
- ✅ **콘텐츠 페이지 4종** (전부 정적 HTML — 크롤러가 JS 없이 읽음, 상호 내비게이션 연결):
  `/guide.html` 사용법 · `/tips.html` 내전 운영 가이드 · `/positions.html` 포지션 밸런싱 · `/faq.html` FAQ
- ✅ **메인(SPA) 크롤러 폴백** — index.html의 #root에 정적 소개 콘텐츠(React 마운트 시 교체) →
  "게시자 콘텐츠 없는 화면" 문제 완화
- ✅ **앱 내 도움말** — 내전 기록 탭 하단 접이식 섹션(HelpSection), 콘텐츠 페이지로 연결
- ✅ **Riot 법적 고지문** — 전 정적 페이지 푸터 + 앱 도움말 하단 (API 신청 요건 겸용)
- 광고 배치 현황: 앱 좌우 사이드바 2개(1200px+) + 콘텐츠 페이지 본문 각 1개.
  빈 화면 단독 광고 없음 — 광고는 항상 실제 콘텐츠 옆에만 배치한다는 원칙 유지
- 📋 이후: 서치 콘솔 색인 → 1~2주 뒤 AdSense 심사 신청, 필요 시 패치 노트/칼럼 추가
